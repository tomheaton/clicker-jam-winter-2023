...
const SlideMenu: React.FC<{
  ...
  const [pinned, setPinned] = useState<boolean>(startPinned ?? false);
  ...
  const buttonClassNameStr = `-z-10 w-20 h-20 top-0 fixed ${
    fromLeft ? "left-0" : "right-0 "
  }`;

  // NOTE: when changing the width and height ('h-[50px]') also change the 'calc()' part
  const pinButtonClassNameStr = `h-[50px] w-[50px] bg-red-500 top-0 absolute ${
    fromLeft ? "left-[calc(100%-50px)]" : "right-[calc(100%-50px)] "
  }`;

  return (
    <div className={menuClassNameStr}>
      {/* 'Show Menu' button */}
